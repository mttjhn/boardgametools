...
function loadLastPlays (userName) {
    ...
    processCollection(bggUser, true);
};
...
function processCollection (userName, owned) {
	...
	$.get( '/api/collection', { username: userName, own: _owned, brief: 0 } )
        .done(loadGameCollection)
        .done(getPlayInfo)
};

function loadGameCollection(data) {
...
function getPlayInfo() {
    ...
    $.each(collectionList, function() {
        $.get('/api/plays', {username: bggUser, id: this.id, page: 1})
            .done(function (playData) {
                console.log('in sub done');
